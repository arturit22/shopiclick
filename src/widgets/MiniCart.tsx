  ...
  if (items.length === 0) {
    return (
      <motion.div
        ...
        className="absolute right-0 top-14 z-[200] w-80 rounded-2xl border border-gray-200 bg-white p-4 shadow-xl"
      >
        ...
        <Link
          onClick={onClose}
          to="/"
          className="mt-4 inline-block rounded-xl border px-4 py-2 text-sm font-medium hover:bg-gray-100"
        >
          Go shopping
        </Link>
      </motion.div>
        ...
        {items.slice(0, 3).map((item) => (
          <div key={item.id} className="flex items-center gap-3">
            ...
            <button
              ...
              className="shrink-0 rounded-md px-2 py-1 text-sm text-gray-400 transition hover:bg-gray-100 hover:text-red-500"
            >
              x
            </button>
          </div>
